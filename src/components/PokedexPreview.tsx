      ...
      <PokedexContainer>
        ...
        <PokemonCard>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'center', gap: '16px', flexDirection: { xs: 'column', sm: 'row' }, alignItems: 'center' }}>
            <PokeSprite src={mockPokemon.image} alt="Imagem do Pokémon" />
          </Box>

          <Box sx={{ marginBottom: '8px' }}>
            <Typography sx={{ color: '#2c4377', marginBottom: '4px' }}>
              <strong>Tipos:</strong> {mockPokemon.types.join(', ')}
            </Typography>
            <Typography sx={{ color: '#2c4377' }}>
              <strong>Fraquezas:</strong> {mockPokemon.fraquezas.length > 0 ? mockPokemon.fraquezas.join(', ') : 'Nenhuma encontrada'}
            </Typography>
          </Box>
          ...
          <Box>
            ...
            {Object.keys(mockPokemon.stats).length > 0 ? (
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: '6px', justifyContent: 'center' }}>
                {Object.entries(mockPokemon.stats).map(([stat, valor]) => (
                  <Box key={stat} sx={{ display: 'inline-block', marginRight: '8px', marginBottom: '4px' }}>
                    <Typography component="span" sx={{ fontWeight: 'bold', color: '#3a5da8', fontSize: { xs: '0.9em', md: '1em' } }}>
                      {stat}:
                    </Typography>
                    <Typography component="span" sx={{ color: '#232a3a', marginLeft: '4px', fontSize: { xs: '0.9em', md: '1em' } }}>
                      {valor}
                    </Typography>
                  </Box>
                ))}
